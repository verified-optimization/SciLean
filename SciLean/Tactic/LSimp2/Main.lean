...
where
  ...
  simpArrow (e : Expr) : M Result := do
    ...
    let q := e.bindingBody!
    let rp ← simp p
    trace[Debug.Meta.Tactic.simp] "arrow [{(← Simp.getConfig).contextual}] {p} [{← isProp p}] -> {q} [{← isProp q}]"
...
@[inline] def withSimpConfig (ctx : Simp.Context) (x : MetaM α) : MetaM α :=
  withConfig (fun c => { c with etaStruct := ctx.config.etaStruct }) <| withReducible x

def main (e : Expr) (ctx : Simp.Context) (usedSimps : UsedSimps := {}) (methods : Methods := {}) : MetaM (Result × UsedSimps) := do
...
def dsimpMain (e : Expr) (ctx : Simp.Context) (usedSimps : UsedSimps := {}) (methods : Methods := {}) : MetaM (Expr × UsedSimps) := do
  withSimpConfig ctx do
    try
...
def simp (e : Expr) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
    (usedSimps : UsedSimps := {}) : MetaM (Simp.Result × UsedSimps) := do profileitM Exception "simp" (← getOptions) do
  match discharge? with
  | none   => LSimp.main e ctx usedSimps (methods := LSimp.DefaultMethods.methods)
  | some d => LSimp.main e ctx usedSimps (methods := { pre := (Simp.preDefault · d), post := (Simp.postDefault · d), discharge? := d })

def dsimp (e : Expr) (ctx : Simp.Context)
    (usedSimps : UsedSimps := {}) : MetaM (Expr × UsedSimps) := do profileitM Exception "dsimp" (← getOptions) do
  LSimp.dsimpMain e ctx usedSimps (methods := LSimp.DefaultMethods.methods)
...
def simpTargetCore (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
...
def simpTarget (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
...
/--
  ...
  This method assumes `mvarId` is not assigned, and we are already using `mvarId`s local context. -/
def simpStep (mvarId : MVarId) (proof : Expr) (prop : Expr) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
...
def simpLocalDecl (mvarId : MVarId) (fvarId : FVarId) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
...
def lsimpGoal (mvarId : MVarId) (ctx : Simp.Context) (discharge? : Option LSimp.Discharge := none)
...
def ldsimpGoal (mvarId : MVarId) (ctx : Simp.Context) (simplifyTarget : Bool := true) (fvarIdsToSimp : Array FVarId := #[])
